...
void font_char_big(char c, unsigned int x, unsigned int y, unsigned char color) {
    ...
    for (unsigned int yy = 0; yy < 8; yy++) {
        for (unsigned int xx = 0; xx < 8; xx++) {
            if (glyph[yy] & (1 << xx)) {
                plot_pixel(x + (xx * 2), y + (yy * 2), color);
                plot_pixel(x + (xx * 2) + 1, y + (yy * 2), color);
                plot_pixel(x + (xx * 2), y + (yy * 2) + 1, color);
                plot_pixel(x + (xx * 2) + 1, y + (yy * 2) + 1, color);
            }
        }
    }
}
